    ...
    <div className="flex items-start justify-between p-4 bg-slate-50 dark:bg-gray-900 hover:bg-slate-100 dark:hover:bg-gray-800 transition-colors">
      ...
      {expandedDrill === index && (
         <div className="p-4 space-y-4 bg-blue-50 dark:bg-blue-950/20 border-l-4 border-blue-400 dark:border-blue-600">
          ...
          {drill.youtube_url && (() => {
            const url = drill.youtube_url;
            // Extract video ID from any YouTube URL format or embed code
            const idMatch = url.match(/(?:v=|\/embed\/|youtu\.be\/|\/v\/)([a-zA-Z0-9_-]{11})/);
            const embedSrc = idMatch ? `https://www.youtube.com/embed/${idMatch[1]}` : null;
            return embedSrc ? (
              <div>
          ...
          <div className="lg:col-span-3">
            <Card className="dark:bg-gray-800 dark:border-gray-700">
              ...
              <CardContent className="pt-6">
                ...
                {/* Drill Sections - warmup, strength, prehab */}
                ...
                {/* Dynamic event drill sections from eventConfig */}
                {ALL_EVENTS.map(event => activeSection === `drills-${event.id}` && (
                  <div key={event.id} className="space-y-4">
                    {getDrillsByEventId(event.id).map((drill, idx) => renderDrillCard(drill, idx))}
                    {isCoach && <Button onClick={() => handleAddDrill(event.id)} variant="outline" className="w-full gap-2 dark:border-gray-600 dark:text-gray-300"><Plus className="w-4 h-4" />Add {event.label} Drill</Button>}
                  </div>
                ))}
